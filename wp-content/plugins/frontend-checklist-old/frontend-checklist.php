...
//check version and prepare message when aoutdated
add_filter('pre_set_site_transient_update_plugins', array('Frontend_Checklist', 'checkVersion')); 
...
class Frontend_Checklist {
	...
	public static function output($atts) {
		...
		if (isset($atts['type']) && $atts['type'] == 'pdf') {
			...
			if (isset($_COOKIE['frontend-checklist-'.$listID])) $sum = $_COOKIE['frontend-checklist-'.$listID];
            else $sum = get_user_meta(get_current_user_id(), 'frontend-checklist-'.$listID, true);
            $_SESSION['frontend-checklist-sum'][$listID] = $sum;
		...
		//HTML checklist
			...
			foreach ($items as $item) {
				if ($item == '') break;
				$output .= '<p style="font-family:Comic\ Sans\ MS;font-size:25px;text-align:center;font-weight:bold; color:#e5b824"><label class="container"><input type="checkbox" id="frontend-checklist-'.$item['fc_listID'].'-item-'.$i.'" onchange="frontend_checklist_checkbox_changed('.$item['fc_listID'].', '.$cookie.', '.$cookie_lifetime_days.')">'.$item['text'].'<span class="checkmark"></span></label></p>';
				$i += 1;
			}
	...
	public static function changedAjax() {
	    ...
	    $sum = (int)$_POST['sum'];
	    $_SESSION['frontend-checklist-sum'][$fc_listID] = $sum;
	    
	    if ((int)$_POST['cookie'] == 1) die;
	...
	public static function update() {
		...
		if ($version != FRONTEND_CHECKLIST_VERSION) {
			...
			//when updating, we have to explicitely change the charset
            $sql = 'ALTER TABLE '.$wpdb->prefix.'fc_lists CHANGE name name VARCHAR(128) CHARACTER SET '.$wpdb->charset.' COLLATE '.$wpdb->collate.' NOT NULL, CHANGE description description VARCHAR(1024) CHARACTER SET '.$wpdb->charset.' COLLATE '.$wpdb->collate.' NOT NULL';
            $wpdb->query($sql);
        
            $sql = 'ALTER TABLE '.$wpdb->prefix.'fc_items CHANGE text text VARCHAR(1024) CHARACTER SET '.$wpdb->charset.' COLLATE '.$wpdb->collate.' NOT NULL';
            $wpdb->query($sql);
			
			//deactivate update infotext
			$meta_options = get_option('frontend-checklist-meta-options');
			$meta_options['hide-infotext'] = 1;
			update_option('frontend-checklist-meta-options', $meta_options);
	...
	public static function activation() {
		...
		$sql = 'CREATE TABLE '.$wpdb->prefix.'fc_items (
					...
					fc_listID int(11) unsigned NOT NULL,
	...
	function checkVersion($transient) {
		...
		$meta_options = get_option('frontend-checklist-meta-options');
		$http_answer = wp_remote_post('https://api.wordpress.org/plugins/info/1.0/frontend-checklist');
		$repository_info = maybe_unserialize($http_answer['body']);
		
		//die (print_r($repository_info, true));
		
		if (version_compare(FRONTEND_CHECKLIST_VERSION, $repository_info->version, '<')) {
		    $user = wp_get_current_user();
		    $first_name = ($user->user_firstname != '') ? $user->user_firstname : $user->user_login;
		    
            $meta_options['infotext'] = sprintf( __('<h3>Frontend Checklist Update Required</h3><p>Hey %s, there is a new version of Frontend Checklist. Please update as soon as possible to ensure the safety and compatibility of your site. Are you annoyed by constantly updating plugins and always worrying about breaking something? Check out my new service <a href="http://www.j-breuer.com/maintenance-power/" target="_blank">Maintenance Power</a>.', 'frontend-checklist'), $first_name );
